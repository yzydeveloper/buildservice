 ...
 */
const runCommand = (command, branchPath = '') => new Promise(async (resolve) => {
 ...
 * @param {*} remote
 */
const putDirectory = async (local, remote) => ssh.putDirectory(local, remote, {
    recursive: true,
    concurrency: 10,
    validate(itemPath) {
 ...
 * @returns
 */
const select = (message = '选择站点', choices = []) => new Promise((resolve, reject) => {
    inquirer
        .prompt([
            {
                type: 'list',
